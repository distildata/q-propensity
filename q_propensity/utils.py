...
# Returns a list of UI objects that show a DAI url running in the UI and provide a link to visit the page
# Usage: q.page['dai'] = ui.form_card(box='3 4 -1 -1', items=dai_content("driverless.h2o.ai"))
def dai_content(dai_url):
    return [ui.text_xl(f'[Driverless AI]({dai_url})'),
            ui.frame(content=' ', height="20px"),  # whitespace
            ui.frame(path=dai_url, height='100%')]